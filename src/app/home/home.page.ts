...
export class HomePage {
  ...
  @ViewChild('vrCanvas', {read : ElementRef}) vrCanvas;

    private width: number = 400;
    private height: number = 250;

    private scene: THREE.Scene = new THREE.Scene();
    private camera: THREE.PerspectiveCamera = new THREE.PerspectiveCamera(75, this.width/this.height, 0.1, 2000);
    ...
    ngAfterViewInit() {
        this.renderer = new THREE.WebGLRenderer({antialias: false, canvas: this.vrCanvas.nativeElement});
        this.controls = new VRControls(this.camera); 
        this.effect = new VREffect(this.renderer);

        //this.renderer.vr = true;
        ...
        video.load();
        video.play();
        document.getElementById("vr-canvas").appendChild(video);
